...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->num_rows > 0) {
        header("Location: ../public/user/register.php?error=username_exists");
                ...
                $hashed_password = password_hash($password, PASSWORD_DEFAULT);         $stmt = $conn->prepare("INSERT INTO users (username, password, email) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $username, $hashed_password, $email);
        if ($stmt->execute()) {
            $new_user_id = $conn->insert_id;                         date_default_timezone_set('Asia/Shanghai');
